...
<ul>
{% for linkable-group in site.data.linkableTypes %}
	{% if linkable-group.name == "Base Protocol JSON structures" %}
		{% for linkableType in linkable-group.children %}
		<li><a href='{{ "/specifications/lsp/3.17/types" | prepend: site.baseurl }}/{{ linkableType.link }}'>{{ linkableType.type }}</a></li>
		{% endfor %}
	{% endif %}
{% endfor %}
</ul>
...
<ul>
{% for linkable-group in site.data.linkableTypes %}
	{% if linkable-group.name == "Basic JSON Structures" %}
		{% for linkableType in linkable-group.children %}
		<li><a href='{{ "/specifications/lsp/3.17/types" | prepend: site.baseurl }}/{{ linkableType.link }}'>{{ linkableType.type }}</a></li>
		{% endfor %}
	{% endif %}
...
<ul>
{% for linkable-group in site.data.linkableTypes %}
	{% if linkable-group.name == "Actual Protocol" %}
		{% for linkableType in linkable-group.children %}
		<li><a href='{{ "/specifications/lsp/3.17/types" | prepend: site.baseurl }}/{{ linkableType.link }}'>{{ linkableType.type }}</a></li>
		{% endfor %}
	{% endif %}
...
The client can use the initialize request to provide `ClientCapabilities` signaling what capabilities it has.

_Response_:
* result: `InitializeResult`
* error.code: `InitializeError.unknownProtocolVersion`
* error.data: `InitializeError`

The server can signal its own `ServerCapabilities` in the `InitializeResult`.
